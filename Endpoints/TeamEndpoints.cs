...
public static class TeamEndpoints {
    public static void MapTeamEndpoints(this IEndpointRouteBuilder app) {
        var group = app.MapGroup("api/v0/team");
        ...
        app.MapGet("api/v0/teams", GetAllTeams);
    }
    ...
    private static async Task<IResult> GetTeamByTeamId(int teamId, ITeamRepo teamRepo, CancellationToken ct) {
        try {
            ...
            var team = await teamRepo.GetTeam(teamId, ct) ?? throw new NullReferenceException("Team object reference is null.");
            return TypedResults.Ok(team);
        }
        catch (Exception ex) {
            Console.WriteLine(ex);
